...
本实验是 L1 冒烟，不注入故障。崩溃与重投场景见 [消费者崩溃与重投](/labs/consumer-crash)。
...
- Publisher Confirm 不表示消费者已处理——两段确认互相独立（见 [可靠性](/brokers/rabbitmq/reliability)）。
- 单节点无副本：Broker 节点磁盘损坏仍可能丢消息；高可用配置见 [存储与高可用](/brokers/rabbitmq/storage-ha)。
