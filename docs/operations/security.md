...
> 本页结论：消息系统的安全基线是三件套——认证（谁在连接）、授权（能碰哪些 Topic/Queue）、传输加密（链路上不可窃听篡改），四个产品的机制名不同但要求等价（规格 §12.3）。本仓库实验环境按约定不开认证、所有端口仅绑定 `127.0.0.1`，这是学习环境的豁免，不是生产建议；生产上线时 Demo 默认账号（如 guest/guest）是明确禁止项。
...
规格与 [实验约定](/guide/lab-conventions) 规定学习环境的安全形态：
