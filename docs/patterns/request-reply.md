...
- 跨语言/跨团队的异步接口，用统一信封（规格 §5.2）承载请求与应答。
...
- **超时 ≠ 失败**。超时时处理方可能仍在处理、可能已处理但应答丢失、也可能从未收到。盲目重发请求会造成重复处理——处理方必须按[幂等消费](/patterns/idempotent-consumer)设计（用 correlationId 或业务键去重）。
