...
实验拓扑与断言细节见[毒消息、重试与 DLQ](/labs/poison-message)。
...
三个机制**不是同一功能的不同配置**，写文档与选型时不得互相替换描述（规格 §11.4 事实检查要求）。
...
- 不保证：重试后消息仍保持原队列顺序；进入 DLQ 的消息会被自动处理；RabbitMQ 队列级 TTL 能表达任意精细的逐条延迟（到期从队头计算，见[毒消息实验](/labs/poison-message)「不保证什么」）。
