...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 局部顺序单元 | ✅ 单个 Queue 内 FIFO：入队顺序即投递顺序（[routing](/products/rabbitmq/routing)） | ✅ 单个 Partition 内按 offset 严格有序（[routing](/products/kafka/routing)） | ✅ FIFO 消息按 MessageGroup：同组有序，组间无序（[routing](/products/rocketmq/routing)） | ✅ 分区内有序；消费侧取决于订阅类型：Key_Shared 同 key 有序，Exclusive/Failover 分区级有序（[routing](/products/pulsar/routing)） | ✅ 单个 Stream 内按 Entry ID 严格有序（无分区，整条日志就是一个顺序单元）（[routing](/products/redis-streams/routing)） | ✅ 单个 Stream 内按 sequence 严格有序；Core NATS 同一发布者连接内按发布顺序送达（[routing](/products/nats/routing)） | ✅ 单个 Queue 内 FIFO；同一 Message Group（_AMQ_GROUP_ID）的消息按序交给同一消费者（[routing](/products/artemis/routing)） | ✅ 单个 Queue 内 FIFO；同一 Message Group（JMSXGroupID）的消息按序交给同一消费者（[routing](/products/activemq-classic/routing)） |
| 同业务 Key 聚到同一单元 | 🔧 把 key 编入 routing key + 绑定规则，属使用约定（[routing](/products/rabbitmq/routing)） | ✅ partition key 哈希，同 key 必进同一分区（[routing](/products/kafka/routing)） | ✅ 发送时指定 MessageGroup（如 orderId）（[routing](/products/rocketmq/routing)） | ✅ 分区 key 哈希 + Key_Shared 按 key 绑定消费者（[routing](/products/pulsar/routing)） | 🔧 无分区可做 key 路由：同 key 天然进同一 Stream（若按 key 拆 Stream），代价是 Stream 数量膨胀（[pitfalls](/products/redis-streams/pitfalls)） | 🔧 同理：可按 Subject 拆分 Stream 聚合 key，无原生 key 哈希（[pitfalls](/products/nats/pitfalls)） | ✅ 发送时指定 _AMQ_GROUP_ID（如 orderId）：同组绑定同一消费者串行（[routing](/products/artemis/routing)） | ✅ 发送时指定 JMSXGroupID（如 orderId）：同组绑定同一消费者串行（[routing](/products/activemq-classic/routing)） |
| 全局顺序 | 🔧 只用一个队列 + 单消费者，牺牲全部并行（[pitfalls](/products/rabbitmq/pitfalls)） | 🔧 单分区 Topic + 单消费者，牺牲全部并行（[pitfalls](/products/kafka/pitfalls)） | 🔧 单 MessageQueue + 顺序消费单线程，牺牲并行（[pitfalls](/products/rocketmq/pitfalls)） | 🔧 单分区 Topic + Exclusive 单消费者（[pitfalls](/products/pulsar/pitfalls)） | 🔧 单 Stream + 单消费者：Stream 本身有序，并行消费即破坏处理顺序（[pitfalls](/products/redis-streams/pitfalls)） | 🔧 单 Stream + 单消费者拉取（[pitfalls](/products/nats/pitfalls)） | 🔧 单队列 + 单消费者，牺牲全部并行（[pitfalls](/products/artemis/pitfalls)） | 🔧 单队列 + 单消费者，牺牲全部并行（[pitfalls](/products/activemq-classic/pitfalls)） |
...
| 产品 | 并行上限 | 顺序代价 |
| :--- | :--- | :--- |
| RabbitMQ | 🔧 队列数 × 每队列消费者数：并行靠拆分队列手动获得（[routing](/products/rabbitmq/routing)） | 需要某 key 有序时只能让该 key 独占一个队列 |
| Kafka | ✅ Consumer Group 内消费者数 ≤ 分区数：并行度由分区数原生决定（[routing](/products/kafka/routing)） | 同 key 消息集中在一个分区，热点 key 无法再拆分（[routing](/products/kafka/routing)） |
| RocketMQ | ✅ 队列数与消费线程数：集群消费模式组内分担队列（[routing](/products/rocketmq/routing)） | FIFO 消费下同一 MessageGroup 串行，挂起会放大延迟 |
| Pulsar | ✅ 取决于订阅类型：Shared 最自由、Key_Shared 按 key 并行、Exclusive/Failover 单活（[routing](/products/pulsar/routing)） | Key_Shared 在 key 倾斜时出现消费者负载不均 |
| Redis Streams | 🔧 组内多消费者竞争分发并行处理；单 Stream 无法再拆分（[routing](/products/redis-streams/routing)） | 并行消费即放弃处理顺序；拆分需业务自建多 Stream 分发规则 |
| NATS | 🔧 一个 Consumer 可由多实例共同拉取；Core Queue Group 并行分发（[routing](/products/nats/routing)） | 同上：并行与顺序互斥，Stream 无分区可拆 |
| Artemis | 🔧 同队列多消费者 round-robin 并行；并行度靠拆分队列手动获得（[routing](/products/artemis/routing)） | Message Group 内串行：同组绑定单消费者，组倾斜时负载不均 |
| ActiveMQ Classic | 🔧 同队列多消费者竞争分发并行；并行度靠拆分队列手动获得（[routing](/products/activemq-classic/routing)） | Message Group 内串行：同组绑定单消费者，组倾斜时负载不均 |
...
- 证据说明：[Kafka 分区与分发](/products/kafka/routing)
