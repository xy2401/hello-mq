...
> 本页结论：六个产品都只提供「某个单元内有序」——RabbitMQ 单队列、Kafka 分区、RocketMQ MessageGroup、Pulsar 分区+订阅类型、Redis Streams 单 Stream、NATS 单 Stream；Redis/NATS 没有分区概念，顺序单元就是整个 Stream。全局顺序只能靠单一顺序单元 + 单消费者换来，且失败重试会不同程度地破坏顺序。
