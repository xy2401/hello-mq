...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 官方管理工具 | ✅ rabbitmqctl/rabbitmq-diagnostics CLI + Management 插件 Web UI 与 HTTP API（[operations](/products/rabbitmq/operations)） | ✅ kafka-topics/kafka-consumer-groups 等 CLI；社区版无官方 Web UI（[operations](/products/kafka/operations)） | ✅ mqadmin CLI + RocketMQ Dashboard（[operations](/products/rocketmq/operations)） | ✅ pulsar-admin/pulsarctl CLI + pulsar-manager Web（[operations](/products/pulsar/operations)） | ✅ redis-cli 全命令族（XINFO/XCLAIM/XGROUP…）；无官方 Web UI（[operations](/products/redis-streams/operations)） | ✅ nats CLI + 服务器监控 HTTP 端点（/varz /jsz /connz）（[operations](/products/nats/operations)） | ✅ artemis CLI（data/tool 子命令）+ hawtio Web 控制台（8161，自带管理 API）（[operations](/products/artemis/operations)） |
| 指标导出 | ✅ 内置 Prometheus 端点与指标 API（[operations](/products/rabbitmq/operations)） | 🔧 JMX 为主，通常需 JMX exporter 转 Prometheus（[operations](/products/kafka/operations)） | 🔧 Broker 指标 + Dashboard 展示，Prometheus 接入需配置（[operations](/products/rocketmq/operations)） | 🔧 Broker 指标 + Prometheus 端点，需配置暴露（[operations](/products/pulsar/operations)） | 🔧 INFO 命令输出为主，Prometheus 需 redis_exporter 等第三方采集（[operations](/products/redis-streams/operations)） | 🔧 监控端点 JSON + prometheus-nats-exporter（官方独立工具）（[operations](/products/nats/operations)） | 🔧 JMX MBean 为主（hawtio 可视化），Prometheus 需 JMX exporter（[operations](/products/artemis/operations)） |

## 关键观测指标

| 指标类别 | RabbitMQ | Kafka | RocketMQ | Pulsar | Redis Streams | NATS | Artemis |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 积压度量 | ✅ 队列消息数 + consumer 未确认数（[operations](/products/rabbitmq/operations)） | ✅ Consumer Lag：按消费组 × 分区（[operations](/products/kafka/operations)） | ✅ 消费堆积：按消费组 × Topic × 队列（[operations](/products/rocketmq/operations)） | ✅ Subscription Backlog：按订阅（[operations](/products/pulsar/operations)） | ✅ XLEN − 组 last-delivered 之后的条目数 + PEL 深度（XINFO STREAM/GROUPS）（[operations](/products/redis-streams/operations)） | ✅ Consumer Pending 数（/jsz 或 nats consumer info）（[operations](/products/nats/operations)） | ✅ 队列 MessageCount（队列深度）+ 已分页消息数（[operations](/products/artemis/operations)） |
| 重投/DLQ 观测 | 🔧 死信队列深度需自行监控（[operations](/products/rabbitmq/operations)） | 🛠 retry/DLT 是普通 Topic，积压需自建告警（[operations](/products/kafka/operations)） | ✅ 重试队列与 %DLQ% 消息数内置可见（[operations](/products/rocketmq/operations)） | 🔧 重投计数与死信 Topic 深度可观测（[operations](/products/pulsar/operations)） | 🔧 XPENDING 可见每条 PEL 消息的投递次数与空闲时间（[operations](/products/redis-streams/operations)） | 🔧 Consumer 的 num_redeliveries 可见；无 DLQ 可观测（[operations](/products/nats/operations)） | ✅ 死信队列深度可观测；deliveryCount 按消息可见（浏览/管理 API）（[operations](/products/artemis/operations)） |
