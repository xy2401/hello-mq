...
- **顺序只在单分区内**：同 key 进同分区才有局部顺序；「Pulsar Topic 没有分区概念」是禁止表述（见 [陷阱](/products/pulsar/pitfalls)）。
...
- 「Shared 订阅同 key 也有序」——不保证任何顺序；同键有序必须 Key_Shared（或单分区 + Exclusive/Failover），这是错误表述（见 [陷阱](/products/pulsar/pitfalls)）。
