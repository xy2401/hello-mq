...
核心实体与关系（详见 [核心概念映射](/products/pulsar/concepts)）：
...
| 重试/DLQ | 客户端 DeadLetterPolicy：negativeAck/ack 超时触发重投，超过 maxRedeliverCount 进 DLQ（[实验](/products/pulsar/reliability)） |
| 延迟消息 | Broker 支持延迟/定时投递（仅 Shared/Key_Shared 订阅生效）；本仓库未实验 |
| 高可用 | ledger 按 quorum 在多 bookie 复制；Broker 无状态可水平扩展；元数据服务需独立保障 |
| 回放 | 原生支持：`reset-cursor` 重置到 earliest/latest/时间戳/MessageId（[实验](/products/pulsar/reliability)） |

## 学习路径

1. [快速开始](/products/pulsar/quick-start)：最短闭环。
2. [核心概念映射](/products/pulsar/concepts)：用 Pulsar 术语回答统一知识模型。
3. [订阅与分发](/products/pulsar/routing)：四种订阅类型与分区 key 路由。
4. [可靠性](/products/pulsar/reliability)：ack 时机、negativeAck、重投与 DLQ。
5. [存储与高可用](/products/pulsar/storage-ha)：ledger、quorum、retention/TTL 与 tiered storage。
6. [运维与观测](/products/pulsar/operations)、[陷阱与检查表](/products/pulsar/pitfalls)。
7. 动手实验：[basic](/products/pulsar/quick-start)、[subscriptions](/products/pulsar/routing)、[redelivery-replay](/products/pulsar/reliability)、[cli-tools](/products/pulsar/operations)。
