...
- **业务事件与订单流**：内置事务消息、延迟/定时消息、消费重试与 DLQ，贴近电商类业务链路（[Docker 验证](/products/rocketmq/DockerTooling)）。
...
| 重试/DLQ | Broker 内置：消费失败按组重试策略重投，达上限进 `%DLQ%<组名>`（[可靠性](/products/rocketmq/reliability)） |
