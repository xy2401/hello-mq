...
- **服务端可靠性策略**：重试次数、重投间隔、死信地址、过期地址都是服务端 address-setting，不依赖客户端自觉（见 [可靠性](/products/artemis/reliability)）。
- **不太适合**：需要按时间任意回放的日志型场景（确认即删除，无保留回放）、超大规模分区吞吐——单个 Queue 不做分区拆分（见 [存储与高可用](/products/artemis/storage-ha)）。

> 边界提示：Artemis 与 [ActiveMQ Classic](/products/activemq-classic/)（6.x）是两套代码库，配置与调优经验不通用；本分卷只讨论 Artemis。
...
核心实体与关系（详见 [核心概念映射](/products/artemis/concepts)）：
...
1. [快速开始](/products/artemis/quick-start)：最短闭环。
2. [核心概念映射](/products/artemis/concepts)：用 Artemis 术语回答统一知识模型。
3. [路由与分发](/products/artemis/routing)：anycast/multicast、divert、selector 与 Message Group。
4. [可靠性](/products/artemis/reliability)：确认后删除、崩溃窗口、重投与死信、去重。
5. [存储与高可用](/products/artemis/storage-ha)：journal、分页、复制与集群边界。
6. [运维与观测](/products/artemis/operations)、[陷阱与检查表](/products/artemis/pitfalls)。
...
- `artemis cli-tools`：纯镜像自带统一入口 `bin/artemis` 完成收发闭环（producer/consumer/browser 子命令，快照已采集，见 [运维与观测](/products/artemis/operations)）。
