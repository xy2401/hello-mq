...
while (running) {
  ...
  note="提交 offset ≠ 业务提交：前者只是消费进度指针，后者才决定重复还是丢失（见三层语义说明 /concepts/delivery-semantics）。"
...
1. **「Kafka 保证全局顺序」**
   真相：顺序只在**分区内**成立。跨分区、再均衡后的交接、多线程消费都可能打乱全局顺序；需要全局顺序只能单分区（牺牲并行度）。本仓库 [ordering-replay 实验](/matrix/experiment/ordering) 验证的是同 key 同分区的局部顺序。
...
3. **「开启事务后，任意外部系统副作用都是 exactly-once」**
   真相：Kafka 事务保证的是 Kafka 内部（produce、sendOffsetsToTransaction）的原子性；数据库、HTTP 等外部副作用不在事务范围内，仍需幂等设计。[idempotence-transaction 实验](/products/kafka/reliability) 验证的也是 Kafka 内部可见性。
