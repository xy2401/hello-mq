...
核心实体与关系（详见 [核心概念映射](/products/nats/concepts)）：
...
1. [快速开始](/products/nats/quick-start)：最短闭环（两个实验分别覆盖 Core 与 JetStream）。
2. [核心概念映射](/products/nats/concepts)：用 NATS 术语回答统一知识模型。
3. [路由与分发](/products/nats/routing)：Subject 通配符、Queue Group 与 Stream 捕获。
4. [可靠性](/products/nats/reliability)：两层的确认边界与崩溃窗口。
5. [存储与高可用](/products/nats/storage-ha)：存储类型、保留策略与 R3 复制。
6. [运维与观测](/products/nats/operations)、[陷阱与检查表](/products/nats/pitfalls)。
...
- `nats cli-tools`：distroless 镜像仅 `/nats-server` 单一二进制，无自带收发 CLI（缺口记录在 gap.out.txt）；状态靠宿主机 8222 监控端点体检（见 [运维与观测](/products/nats/operations)）。
