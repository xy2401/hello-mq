...
Explicit ACK + 并行处理时，先投递的消息若后 ACK（或触发重投），**处理完成顺序**不再等于 Stream 序列顺序。需要按业务键有序时：单消费者串行处理，或用 Message Group 语义的替代设计（JetStream 无 Kafka 分区键机制，见 [路由与分发](/products/nats/routing)）。
