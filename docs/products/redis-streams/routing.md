...
`basic` 实验中的组内单消费者路径见 [快速开始](/products/redis-streams/quick-start)；多组广播可通过在同一 Stream 上创建第二个组并重复消费复现：
...
- 单个 Stream 的写入吞吐受单节点、单 key 限制，路由设计不能替代水平扩展（见 [存储与高可用](/products/redis-streams/storage-ha)）。
