...
- **不太适合**：大规模长期保留与回放（受单机内存约束）、跨多 Broker 的水平分区日志、多租户平台——单个 Stream Key 不会自动分裂成分区（见 [存储与高可用](/brokers/redis-streams/storage-ha)）。

> 边界提示：本分卷只讨论 Streams。Redis Pub/Sub 是易失的广播通道，不是可靠队列，二者不可互相替代（对比 NATS Core 与 JetStream 的分层，见 [/brokers/nats/](/brokers/nats/)）。
...
核心实体与关系（详见 [核心概念映射](/brokers/redis-streams/concepts)）：
...
1. [快速开始](/brokers/redis-streams/quick-start)：最短闭环。
2. [核心概念映射](/brokers/redis-streams/concepts)：用 Redis Streams 术语回答统一知识模型。
3. [路由与分发](/brokers/redis-streams/routing)：Stream key、多组广播与组内竞争。
4. [可靠性](/brokers/redis-streams/reliability)：XACK、PEL 与崩溃窗口。
5. [存储与高可用](/brokers/redis-streams/storage-ha)：保留策略、持久化与复制边界。
6. [运维与观测](/brokers/redis-streams/operations)、[陷阱与检查表](/brokers/redis-streams/pitfalls)。
...
- `redis-streams cli-tools`：纯镜像自带 `redis-cli` 六件套完成 XADD/XREADGROUP/XACK 闭环（见 [运维与观测](/brokers/redis-streams/operations)）。
