...
- 用 NACK+requeue 当无限重试：毒消息卡队头，CPU 空转。→ 用 [TTL+DLX 重试环](/labs/poison-message)。
...
- [ ] 幂等：`messageId` 唯一键表与业务写入同事务；已验证重投场景（本仓库 [consumer-crash 实验](/labs/consumer-crash) 可复用为验收用例）。
