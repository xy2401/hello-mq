...
<VersionBadge product="RabbitMQ" broker="4.1.4" client="amqp-client 5.34.0" image="tag+digest@.env.versions" />
...
- **不太适合**：超长保留期的日志回放、按任意位点重读历史——消息在 ACK 后即从队列移除，这是队列语义而非日志语义（对比 Kafka，见 [消息模型](/fundamentals/models)）。
...
核心实体与关系（详见 [核心概念映射](/brokers/rabbitmq/concepts)）：
...
| 重试/DLQ | 无内置消费重试；TTL + DLX 组合模式（[实验](/labs/poison-message)） |
...
1. [快速开始](/brokers/rabbitmq/quick-start)：最短闭环。
2. [核心概念映射](/brokers/rabbitmq/concepts)：用 RabbitMQ 术语回答统一知识模型。
3. [路由与分发](/brokers/rabbitmq/routing)：Exchange 类型与绑定。
