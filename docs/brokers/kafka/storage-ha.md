...
- 记录写入后按 offset 编号；消费者只移动自己的位点，**日志不随消费删除**——这是与队列型 Broker 的根本区别（见 [消息模型](/fundamentals/models)）。
...
- 分区数决定并行度上限，且 key 分布随分区数变化——**扩分区不可逆**，先想清楚（见 [分区与分发](/brokers/kafka/routing)）。
