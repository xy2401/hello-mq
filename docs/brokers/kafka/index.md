...
<VersionBadge product="Apache Kafka" broker="4.3.1" client="kafka-clients 4.3.1" image="tag+digest@.env.versions" />
...
- **事件流与回放**：消息按 retention 保留，多个消费组可各自从头重读（[实验](/labs/ordering)）。
- **高吞吐持久化管道**：顺序写盘 + 批量 + 零拷贝，适合大量事件的接入与分发。
- **同键有序的任务流**：同一 key 的消息进同一分区，分区内顺序消费。
- **不太适合**：单条消息级别的灵活路由（没有 Exchange/Binding 概念）、需要「消费即删除」的竞争队列语义——Kafka 的删除由 retention 决定，与消费进度无关（对比 RabbitMQ，见 [消息模型](/fundamentals/models)）。
...
核心实体与关系（详见 [核心概念映射](/brokers/kafka/concepts)）：
...
| 回放 | 原生支持：按 offset/时间戳重置消费位点（[实验](/labs/ordering)） |

## 学习路径

1. [快速开始](/brokers/kafka/quick-start)：最短闭环。
2. [核心概念映射](/brokers/kafka/concepts)：用 Kafka 术语回答统一知识模型。
3. [分区与分发](/brokers/kafka/routing)：key、分区分配与消费组。
