...
| :--- | :--- | :--- |
| `pulsar_msg_backlog`（按订阅） | 该订阅未消费消息数 | 持续增长 = 消费跟不上生产（积压），见 [背压与积压](/fundamentals/backpressure) |
...
- **reset-cursor 是双刃剑**：回放会把 backlog 瞬间拉满并触发重复投递，执行前确认消费端幂等（[redelivery-replay 实验](/brokers/pulsar/reliability) 即此场景）。
