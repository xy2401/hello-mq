...
Pulsar 的「分发」= 订阅模型 + 分区 key 路由：广播靠多个订阅，竞争消费靠 Shared，见专页 [订阅与分发](/brokers/pulsar/routing)。

### 4. 存储与保留

消息写入 BookKeeper 的 ledger，消费不删除数据。两个互不等价的开关（详见 [存储与高可用](/brokers/pulsar/storage-ha)）：
...
`send()` 同步返回 MessageId 表示消息已按 ack quorum 持久化；异步发送必须处理失败回调。Producer 网络重试可能产生重复，Broker 侧消息去重（deduplication）默认关闭——本仓库以幂等消费兜底，见 [可靠性](/brokers/pulsar/reliability)。
...
- exactly-once：事务可实现 **Pulsar 内部**跨 Topic/分区的原子发布与原子 ack；跨外部系统仍需幂等消费（见 [可靠性](/brokers/pulsar/reliability)）。

### 8. 顺序语义

顺序只在**单分区**内成立。分区 Topic 中同 key 进同一分区；消费侧 Exclusive/Failover 保持单分区顺序，Key_Shared 保持同 key 顺序，**Shared 不保证任何顺序**（规格 §7.4 禁止表述，见 [陷阱](/brokers/pulsar/pitfalls)）。详见 [顺序语义](/fundamentals/ordering)。
...
Broker 无状态水平扩展；ledger 按 quorum 复制容忍 bookie 故障；Namespace 可配置跨集群 geo-replication。扩容方式与容量约束见 [存储与高可用](/brokers/pulsar/storage-ha)。
...
见专页 [陷阱与检查表](/brokers/pulsar/pitfalls)。
