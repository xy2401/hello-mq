...
- 匿名访问是本实验的默认配置，不是生产配置；安全基线见 [运维与观测](/brokers/activemq-classic/operations)。
...
容器运行期间可用 Web 控制台观察 Queue/Topic 与连接：浏览器打开 `http://127.0.0.1:8161`，默认凭据 admin/admin（镜像 jetty realm）。命令行侧的深度清点用 `QueueBrowser`（本仓库 `stats` 命令即用它）；镜像自带的统一入口 `bin/activemq` 及 producer/consumer/status 等任务见 [运维与观测](/brokers/activemq-classic/operations) 的 cli-tools 小节。指标读法同页。

## 清理与安全

- run.sh 退出时的 `docker compose down --volumes` 只 down 本实验的 Compose Project（`hello-mq-activemq-classic-basic`），不会碰其他容器或卷。
- 本仓库的实验 Classic 不挂持久卷：KahaDB 数据随容器销毁；生产环境的持久化与高可用取舍见 [存储与高可用](/brokers/activemq-classic/storage-ha)。

## 下一步

- 概念不熟：读 [核心概念映射](/brokers/activemq-classic/concepts)。
