...
| ActiveMQ.DLQ 深度 | 全 broker 共享死信的消息数 | **非零即告警**：毒消息或系统性失败；无法区分业务线（见 [陷阱](/brokers/activemq-classic/pitfalls)） |
...
- 默认配置下 61616 匿名可连、控制台 admin/admin——观测能力不设防，安全基线必须先于生产上线（见 [陷阱与检查表](/brokers/activemq-classic/pitfalls)）。
