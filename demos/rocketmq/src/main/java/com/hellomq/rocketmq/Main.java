...
public final class Main {
  ...
  public static void main(String[] args) throws Exception {
    if (args.length == 0) {
      System.err.println("usage: hello-mq-rocketmq <setup|produce|consume|inspect-db> [--key=value ...]");
      System.exit(2);
    }
    ...
    switch (command) {
      case "setup" -> {
        // Topic/消费组由 lab 编排经 mqadmin 创建；这里仅记录拓扑意图，保持四产品 CLI 对称。
        LabLogger log = LabLogger.of("setup", "rocketmq", parsed.get("lab", "unknown"), "order-service");
