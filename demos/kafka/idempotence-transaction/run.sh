#!/usr/bin/env bash
# kafka/idempotence-transaction：提交事务 3 条对 read_committed 消费者可见，中止事务 2 条不可见。
# Kafka 管理 CLI 的原始 lag 仍包含不可见的中止事务控制记录，本 fixture 稳定为 1。
set -euo pipefail
...
assert_eq "rawLagIncludesAbortedControlRecord" "$(kafka_group_lag orders-txn-group)" 1
